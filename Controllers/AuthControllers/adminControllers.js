...
const GetAddTheAdmin = async (req, res) => {
  ...
  if (isAdmin)
    return res
      .status(409)
      .json({ error: true, message: "Admin Is Already Registerd" });
...
const AdminLoginApi = async (req, res) => {
  ...
  // compare password
  const isMatched = comparePassword(
    req.body.password,
    isUser.password,
    isUser.secretKey
  );
...
const UpdateAdmin = async (req, res) => {
  ...
  try {
    const Update = await AdminModel.findByIdAndUpdate(id, formdata, {
      new: true,
    });
    if (!Update)
      return res.status(400).json({ error: true, message: "Not Updated" });
...
const AdminForgotPassword = async (req, res) => {
  ...
  const mailOptions = {
    ...
    html: EmailForResetLink(
      isUser.name,
      `https://admin.hoteliorooms.com/reset-password/${resetUrl}`
    ),
  };
