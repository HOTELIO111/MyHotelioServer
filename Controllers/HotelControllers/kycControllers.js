...
const {
  ...
  GetAllKyc,
  IsVerifiedActions,
} = require("../../helper/vendor/kycHelpers");
...
const RegisterKyc = async (req, res) => {
  try {
    // Request validation
    const { vendorId, name, email, aadharNo, panNo, aadharImg, panImg } =
      req.body;
    if (
      !vendorId ||
      !name ||
      ...
      return res
        .status(400)
        .json({ error: true, message: "Invalid request data" });
    }
    // Find the vendor
    ...
    if (existingKycRequest) {
      ...
      if (existingKycRequest.isVerified === "failed") {
        ...
        const isUpdate = await KycModel.findByIdAndUpdate(
          existingKycRequest._id,
          {
            vendorId,
            name,
            email,
        ...
        return res
          .status(200)
          .json({ error: false, message: "success", data: isUpdate });
      }
    }
    ...
    if (!isRequested) {
      return res
        .status(500)
        .json({ error: true, message: "Failed to create KYC request" });
    }

    res
      .status(200)
      .json({ error: false, message: "Success", data: isRequested });
  } catch (error) {
...
const UpdateKyc = async (req, res) => {
  try {
    ...
    if (!vendorId) {
      return res
        .status(400)
        .json({ error: true, message: "vendorId is required" });
    }
    ...
    const existingKycRequest = await KycModel.findOne({ vendorId });
    if (!existingKycRequest) {
      return res.status(404).json({
        error: true,
        message: "No existing KYC request found for this vendorId",
      });
    }

    if (existingKycRequest.isVerified !== "approved") {
      return res.status(409).json({
        error: true,
        message: `You cannot update KYC as it is currently ${existingKycRequest.isVerified}`,
      });
    }

    const updatePayload = {
      ...existingKycRequest.toObject(),
      isVerified: "requested",
    };

    if (name) updatePayload.name = name;
    if (email) updatePayload.email = email;
    if (aadharNo) updatePayload.aadharNo = aadharNo;
    if (panNo) updatePayload.panNo = panNo;
    if (aadharImg) updatePayload.aadharImg = aadharImg;
    if (panImg) updatePayload.panImg = panImg;

    const updatedKycRequest = await KycModel.findByIdAndUpdate(
      existingKycRequest._id,
      updatePayload,
      { new: true }
    );

    if (!updatedKycRequest) {
      return res
        .status(500)
        .json({ error: true, message: "Failed to update KYC request" });
    }
...
const deleteTheKycRequest = async (req, res) => {
  ...
  try {
    ...
    if (!response)
      return res.status(404).json({ error: true, message: "try again" });
...
const MakeActionKyc = async (req, res) => {
  ...
  if (!["approved", "failed", "requested"].includes(action))
    return res
      .status(401)
  ...
  try {
    if (!id && !action)
      return res.status({
        error: true,
        message: "please check the credentials try again",
      });
    ...
    if (!response)
      return res
        .status(404)
        .json({ error: true, message: "Kyc Request not found" });
