...
const DeleteTheSingleVendor = async (id) => {
  try {
    const [vendorDeleted, HotelsDeleted, kyc] = await Promise.all([
      VendorModel.findByIdAndDelete(id),
      HotelModel.deleteMany({ vendorId: id }),
      KycModel.findOneAndDelete({ vendorId: id }),
    ]);
...
const DeleteAllVendor = async () => {
  try {
    const [allvendorDeleted, allHotelsDeleted] = await Promise.all([
      ...
      HotelModel.deleteMany({ isAddedBy: "vendor" }),
    ]);
