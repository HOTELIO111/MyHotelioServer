...
const AddFacility = async (req, res) => {
  ...
  try {
    const isAvailable = await FacilitiesModel.findOne({
      title: formdata.title,
    });
    if (isAvailable)
      return res
        .status(409)
        .json({ error: true, message: "Already Registered" });
    ...
    if (!isReg)
      return res
        .status(400)
        .json({ error: true, message: "Register Failed ! please try again" });
...
const DeleteFacilities = async (req, res) => {
  ...
  FacilitiesModel.findByIdAndDelete(id)
    .then(() => {
  ...
  try {
    ...
    if (formdata.title !== facData.title) {
      ...
      if (oldOne)
        return res
          .status(409)
          .json({ error: true, message: "already Amenity is available" });
    }

    // now update the data
    const isUpdated = await FacilitiesModel.findByIdAndUpdate(id, formdata, {
      new: true,
    });
    if (!isUpdated)
...
const DeleteAllFacility = async (req, res) => {
  FacilitiesModel.deleteMany({})
    .then(() => {
      res
        .status(200)
        .json({ error: false, message: "All Amenity deleted successfully " });
    })
    .catch((error) => {
      res.status(500).json({ error: true, message: error.message });
    });
